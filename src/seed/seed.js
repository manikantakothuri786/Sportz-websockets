...
async function insertCommentary(matchId, entry) {
    ...
    // Map data file's "minute" (singular) to API's "minutes" (plural)
    const minuteValue = entry.minutes !== undefined ? entry.minutes : entry.minute;
    if (Number.isFinite(minuteValue)) {
        payload.minutes = minuteValue;
    }
    // Only add sequence if it's a valid number
    ...
    if (!response.ok) {
        const text = await response.text();
        throw new Error(`Failed to create commentary for match ${matchId}: ${response.status} - ${text}`);
    }
    const responsePayload = await response.json();
    return responsePayload.data;
}
...
function normalizeCricketFeed(entries, match) {
    const sorted = [...entries].sort((a, b) => {
        ...
        const minA = Number.isFinite(a.minutes) ? a.minutes : (Number.isFinite(a.minute) ? a.minute : Number.MAX_SAFE_INTEGER);
        const minB = Number.isFinite(b.minutes) ? b.minutes : (Number.isFinite(b.minute) ? b.minute : Number.MAX_SAFE_INTEGER);
        return minA - minB;
    });
